...
public class ShotgunFire : WeaponEffects, IFireable
{
    ...
    public override void WeaponFire()
    {
        ...
        Debug.Log("WeaponFire called at: " + Time.time);
        Fire(ammo);
        WeaponFireEffects();
        CheckForHit();
    ...
    private void ShootEnemy(RaycastHit hit)
    {
        float weaponForce = UnityEngine.Random.Range(weaponData.minForce, weaponData.maxForce);
        bool direction = IsEnemyFacing(playerHolding, hit.collider.transform);
        IDamagable enemey = hit.collider.GetComponent<IDamagable>();

        if (anim.GetFloat(AIM) < 0.1f && anim.GetFloat(AIM) > -0.1f)
        {
            enemey.Damage(weaponData.fowardDamage, splatters, weaponData.torso, weaponData.removeTorso, direction, true, weaponForce);
        }
        if (anim.GetFloat(AIM) > 0.1f)
        {
            enemey.Damage(weaponData.upWardDamage, splatters, weaponData.head, weaponData.removeHead, direction, false, 0);
        }
        if (anim.GetFloat(AIM) < -0.1f)
        {
            enemey.Damage(weaponData.downWardDamage, splatters, weaponData.legs, weaponData.removeLegs, direction, false, 0);
        }
    }
